...
import io.kaitai.struct.languages.{GoCompiler, RustCompiler, NimCompiler, WiresharkCompiler}
...
object Main {
  ...
  def compile(specs: ClassSpecs, spec: ClassSpec, lang: LanguageCompilerStatic, conf: RuntimeConfig): CompileLog.SpecSuccess = {
    ...
    val cc = lang match {
      ...
      case WiresharkCompiler =>
        new WiresharkClassCompiler(specs, spec, config)
      case _ =>
